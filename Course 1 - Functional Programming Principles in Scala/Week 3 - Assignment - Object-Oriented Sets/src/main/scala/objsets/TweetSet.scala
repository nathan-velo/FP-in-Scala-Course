...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet =
    def max(tweetOne: Tweet, tweetTwo: Tweet): Tweet =
      if tweetOne.retweets > tweetTwo.retweets then tweetOne else tweetTwo

    max(
      elem,
      max(
        if !left.isEmpty then left.mostRetweeted else elem,
        if !right.isEmpty then right.mostRetweeted else elem
      )
    )
